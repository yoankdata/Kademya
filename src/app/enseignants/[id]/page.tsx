  ...
  return (
    <>
      {Array.from({ length: fullStars }).map((_, i) => (
        <Star
          key={`full-${i}`}
          className="w-4 h-4 inline-block text-yellow-500 fill-yellow-500"
        />
      ))}
      {Array.from({ length: emptyStars }).map((_, i) => (
        <Star
          key={`empty-${i}`}
          className="w-4 h-4 inline-block text-gray-300 fill-gray-100 dark:fill-gray-800"
        />
      ))}
    </>
  );
...
export default async function TeacherProfilePage({ params }: PageProps) {
  ...
  // On ne montre que les profs avec abonnement actif ET vérifiés
  if (!data || !data.abonnement_actif || !data.verifie) {
    notFound();
  }
          ...
          <div className="grid gap-6 md:grid-cols-[minmax(0,2fr)_minmax(0,1fr)] md:items-center">
            ...
            <div className="space-y-3">
              <div className="flex items-center gap-3 flex-wrap">
                ...
                {prof.verifie && (
                  <Badge className="bg-green-600/10 text-green-700 dark:bg-green-300/20 dark:text-green-300 border border-green-700/30 pointer-events-none">
                ...
                <div className="space-y-3 text-sm">
                  <div className="flex justify-between items-center">
                    <span className="font-semibold text-foreground flex items-center gap-2">
                      <Star className="w-4 h-4 text-yellow-500" /> Matière
                      enseignée :
                    </span>
                  ...
                  <div className="flex justify-between items-center">
                    <span className="font-semibold text-foreground flex items-center gap-2">
                      <Star className="w-4 h-4 text-yellow-500" /> Niveau scolaire
                      :
                    </span>
            ...
            <Card className="border-l-4 border-primary rounded-xl shadow-inner bg-primary/5">
              <CardContent className="p-4 text-sm text-muted-foreground space-y-2">
                <p className="font-bold text-primary flex items-center gap-2">
                  <MessageSquare className="w-4 h-4" /> Conseils pour le premier
                  message :
                </p>
          ...
          <div className="space-y-8">
            ...
            <Card className="shadow-lg rounded-2xl">
              <CardContent className="p-6 md:p-8 space-y-4">
                ...
                <div className="text-base text-muted-foreground leading-relaxed whitespace-pre-wrap break-words">
                  {prof.biographie ||
              ...
              {avis.length === 0 ? (
                <Card className="border-dashed bg-muted/20 p-6 text-center">
                  <p className="text-base text-muted-foreground">
                    Soyez le premier à laisser un avis ! Votre feedback sera
                    validé après un cours.
                  </p>
                </Card>
              ) : (
            ...
            <section className="bg-primary/5 rounded-2xl p-6 md:p-8 border border-primary/20 shadow-inner">
              ...
              <p className="text-sm text-muted-foreground mb-6">
                Votre avis est important ! Il sera publié après vérification
                par notre équipe pour assurer l&apos;authenticité de
                l&apos;expérience.
              </p>
              ...
              <form action={ajouterAvisAction} className="space-y-5">
                ...
                <div className="space-y-1.5">
                  <label className="text-sm font-semibold text-foreground">
                    Votre contact pour vérification (Non Public)
                  </label>
                  <input
                    name="parent_contact"
                    required
                  ...
                  <p className="text-xs text-muted-foreground mt-1">
                    Nous l&apos;utiliserons pour valider votre cours avec le
                    professeur.
                  </p>
                </div>
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="relative">
                    ...
                    >
                      ...
                      <option value="5">
                        5/5 - Excellent, je recommande vivement
                      </option>
                      <option value="4">
                        4/5 - Très bon, satisfait de l&apos;enseignement
                      </option>
                      <option value="3">3/5 - Correct, service attendu</option>
                      <option value="2">
                        2/5 - Moyen, quelques réserves
                      </option>
                      <option value="1">
                        1/5 - Mauvais, très insatisfait
                      </option>
                    </select>
                ...
                <div className="space-y-1.5">
                  ...
                  <textarea
                    ...
                    className="w-full border border-input rounded-lg px-4 py-2.5 text-sm bg-background focus:border-primary focus:ring-1 focus:ring-primary break-words whitespace-pre-wrap"
                  />
                </div>
